...
const SUPABASE_URL = 'https://ixomybhhugfhqowlklep.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml4b215YmhodWdmaHFvd2xrbGVwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI2ODYxNzksImV4cCI6MjA5ODI2MjE3OX0.zzq91glpxy69VLze2iABHaaWSAY1R--JaMZzUjMr9UU';
...
async function initStudentViews() {
    ...
    if (gradesError || attendanceError) {
        reportBody.innerHTML = '<tr><td colspan="8">Error fetching academic records</td></tr>';
        console.error("Grades Error:", gradesError, "Attendance Error:", attendanceError);
        return;
    }
    ...
    grades.forEach(g => {
        ...
        // 3. Calculate Present/Absent for this specific course
        const courseLogs = attendanceLogs ? attendanceLogs.filter(log => log.course_id === g.course_id) : [];
        const totalP = courseLogs.filter(log => log.status === 'Present').length;
        const totalA = courseLogs.filter(log => log.status === 'Absent').length;
        const recordStr = `<span style="color:green; font-weight:600;">P: ${totalP}</span> | <span style="color:red; font-weight:600;">A: ${totalA}</span>`;
