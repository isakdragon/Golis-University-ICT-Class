...
if (saveGradesBtn) {
    saveGradesBtn.addEventListener('click', async function () {
        ...
        gradeInputs.forEach(input => {
            ...
            if (!gradesData[sid]) {
                gradesData[sid] = { student_id: sid, attendance: 0, assignment: 0, mid_exam: 0, final_exam: 0 };
            }
            gradesData[sid][field] = val;
        });

        const gradesArray = Object.values(gradesData);

        setButtonBusy(saveGradesBtn, 'Saving Grades...');
        try {
            // Assuming you have a similar RPC function for saving grades
            const { error } = await supabase.rpc('secure_teacher_save_grades', {
                teacher_id: currentUser.id,
                teacher_pass: currentUser.password,
                p_course_id: currentUser.course_id,
            ...
            if (error) {
                ...
                alert('Academic grades successfully stored!');
            }
        } catch (err) {
            console.error('Grade save error:', err);
        } finally {
            setButtonBusy(saveGradesBtn, null, 'Save Academic Grades');
        }
    });
}
